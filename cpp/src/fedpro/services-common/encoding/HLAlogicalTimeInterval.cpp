...
namespace RTI_NAMESPACE
{
   ...
   {
   public:
      explicit HLAlogicalTimeIntervalImplementation(const RTIambassador * ambassador)
            : _ambassador(ambassador)
      {
         _theTime = _ambassador->getTimeFactory()->makeZero();
      }

      unique_ptr <LogicalTimeInterval> _theTime;
      const RTIambassador * _ambassador;

      ~HLAlogicalTimeIntervalImplementation() = default;
   };

   HLAlogicalTimeInterval::HLAlogicalTimeInterval(HLAlogicalTimeInterval const & rhs)
   {
      _impl = new HLAlogicalTimeIntervalImplementation(rhs._impl->_ambassador);
      *_impl->_theTime = *rhs._impl->_theTime;
   }

   HLAlogicalTimeInterval::HLAlogicalTimeInterval(const RTIambassador * ambassador)
   {
      _impl = new HLAlogicalTimeIntervalImplementation(ambassador);
   }
   ...
   {
      _impl = new HLAlogicalTimeIntervalImplementation(ambassador);
      *_impl->_theTime = logicalTimeInterval;
   }

   HLAlogicalTimeInterval & HLAlogicalTimeInterval::set(const LogicalTimeInterval & value)
   {
      *_impl->_theTime = value;
      return *this;
   }

   void HLAlogicalTimeInterval::get(LogicalTimeInterval & theTime)
   {
   ...
   bool HLAlogicalTimeInterval::isSameTypeAs(const DataElement & other) const
   {
   ...
   void HLAlogicalTimeInterval::encode(
   ...
   void HLAlogicalTimeInterval::encodeInto(
   ...
   size_t HLAlogicalTimeInterval::getEncodedLength() const
   {
      return 4 + _impl->_theTime->encodedLength();
   }

   // Decodes this element from the ByteWrapper.
   HLAlogicalTimeInterval & HLAlogicalTimeInterval::decode(
   ...
   size_t HLAlogicalTimeInterval::decodeFrom(
         vector <Octet> const & buffer,
         size_t startIndex)
   {
      size_t index = startIndex;
      if (index + 4 > buffer.size()) {
         throw EncoderException(L"HLAlogicalTimeInterval cannot decode (indata too short)");
      }
