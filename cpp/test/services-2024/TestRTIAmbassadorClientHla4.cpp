...
TEST(TestRTIambassadorClientHla4, rtiConfigurationParseCorrectly_When_NoSettingsAreProvided)
{
   ...
   // When
   std::unique_ptr<rti1516_202X::fedpro::RtiConfiguration>
         rtiConfiguration = clientAdapter.parseRtiConfiguration(providedConfiguration, inputList);
...
TEST(TestRTIambassadorClientHla4, rtiConfigurationParseCorrectly_When_OnlySettingsAreProvided)
{
   ...
   EXPECT_EQ(rtiConfiguration->rtiaddress(), "");
   EXPECT_EQ(rtiConfiguration->configurationname(), "");
   EXPECT_EQ(rtiConfiguration->additionalsettings(), "FedPro.connect.timeout=10,\nLRC.skipVersionCheck=false");
}
