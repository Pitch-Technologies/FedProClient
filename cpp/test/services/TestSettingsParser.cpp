...
protected:
   ...
   void verifyThatNoSettingHasValue(const Properties & settings)
   {
      EXPECT_EQ(std::chrono::seconds(123),
                settings.getDuration(SETTING_NAME_HEARTBEAT_INTERVAL, std::chrono::seconds(123)));
      EXPECT_EQ(std::chrono::seconds(123),
                settings.getDuration(SETTING_NAME_RESPONSE_TIMEOUT, std::chrono::seconds(123)));
      EXPECT_EQ(std::chrono::seconds(123),
                settings.getDuration(SETTING_NAME_RECONNECT_LIMIT, std::chrono::seconds(123)));
      EXPECT_EQ(false, settings.getBoolean(SETTING_NAME_ASYNC_UPDATES, false));
      EXPECT_EQ(randomString, settings.getString(SETTING_NAME_CONNECTION_HOST, randomString));
      EXPECT_EQ(4545, settings.getUnsignedInt32(SETTING_NAME_CONNECTION_MAX_RETRY_ATTEMPTS, 4545));
      EXPECT_EQ(randomUint16, settings.getUnsignedInt16(SETTING_NAME_CONNECTION_PORT, randomUint16));
      EXPECT_EQ(randomString, settings.getString(SETTING_NAME_CONNECTION_PROTOCOL, randomString));
      EXPECT_EQ(std::chrono::seconds(123),
                settings.getDuration(SETTING_NAME_CONNECTION_TIMEOUT, std::chrono::seconds(123)));
      EXPECT_EQ(-123456, settings.getInt(SETTING_NAME_CONSOLE_LOG_LEVEL, -123456));
      EXPECT_EQ(-123456, settings.getInt(SETTING_NAME_ROTATING_FILE_LOG_LEVEL, -123456));
      EXPECT_EQ(randomString, settings.getString(SETTING_NAME_ROTATING_FILE_LOG_PATH, randomString));
      EXPECT_EQ(4545, settings.getUnsignedInt32(SETTING_NAME_MESSAGE_QUEUE_SIZE, 4545));
      EXPECT_EQ(false, settings.getBoolean(SETTING_NAME_RATE_LIMIT_ENABLED, false));
   }
...
TEST_F(TestSettingsParser, parse_Given_NoSettingsLine)
{
   ...
   // Then
   verifyThatNoSettingHasValue(clientSettings);
}
...
TEST_F(TestSettingsParser, parse_Given_HostnameLocalhostInSettingsLine)
{
   // Given
   string_view settingsLine{"connect.hostname=localhost"};
   // When
   Properties settings = SettingsParser::parse(settingsLine);
   // Then
   EXPECT_EQ("localhost", settings.getString(SETTING_NAME_CONNECTION_HOST, ""));
}
...
TEST_F(TestSettingsParser, SettingsParseDefaults_When_EnvVarsHaveNoValues)
{
   ...
   verifyThatNoSettingHasValue(SettingsParser::parse());
}
