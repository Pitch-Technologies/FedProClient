...
TEST_F(TestRTIAmbassador, connectWebsocketUnknownHost)
{
   ...
   rtiConfig.withRtiAddress(L"unknown.host.invalid").withAdditionalSettings(L"FedPro.connect.protocol=websocket");
   auto rtiAmbassador = createRTIambassador();
   auto federateAmbassador = std::make_unique<RTI_NAMESPACE::NullFederateAmbassador>();

   // Test twice to ensure that failed attempts have no side effects, such as a leftover PersistentSession,
   // that could affect following attempts.
