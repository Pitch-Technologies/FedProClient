...
public abstract class RTIambassadorClientGenericBase {
   ...
   protected boolean _asyncUpdates = Boolean.getBoolean("thinhla.asyncUpdates");
   ...
   private static class QueuedCallback
   {
      ...
      public QueuedCallback(CallbackRequest callbackRequest,
                            int sequenceNumber,
   ...
   protected boolean evokeCallbackBase(double approximateMinimumTimeInSeconds) throws FedProRtiInternalError
   {
   ...
   protected boolean evokeMultipleCallbacksBase(double minimumTime,
                                                double maximumTime) throws FedProRtiInternalError
   {
      synchronized (_connectionStateLock) {
         if (_callbackThread != null) {
            // Immediate mode
   ...
   protected void enableCallbacksBase() throws FedProRtiInternalError
   {
      try {
         synchronized (_callbackLock) {
   ...
   protected void disableCallbacksBase() throws FedProRtiInternalError
   {
      try {
         synchronized (_callbackLock) {
   ...
   protected CallResponse doHlaCallBase(CallRequest callRequest) throws FedProRtiInternalError, FedProNotConnected
   {
      try {
         PersistentSession session = getSession();
         if (session == null) {
   ...
   protected CompletableFuture<CallResponse> doAsyncHlaCallBase(CallRequest callRequest) throws FedProNotConnected
   {
      try {
         PersistentSession session = getSession();
         if (session == null) {
   ...
   protected abstract void throwOnException(CallResponse callResponse) throws FedProRtiInternalError;

   private static void sendHlaCallbackErrorResponse(PersistentSession resumingClient,
                                                    int sequenceNumber,
                                                    Throwable throwable)
   {
   ...
   private void hlaCallbackRequest(int sequenceNumber,
                                   byte[] hlaCallback)
   {
      try {
         ...
         LOGGER.warning(() -> String.format("Failed to parse callback request with sequence number %d: %s",
            sequenceNumber,
            e));
   ...
   @GuardedBy("_connectionStateLock")
   protected PersistentSession createPersistentSession(ConnectSettings connectSettings) throws FedProRtiInternalError
   {
      final long heartBeatIntervalMillis = getTimeoutSettingMillis(SETTING_NAME_HEARTBEAT_INTERVAL,
         DEFAULT_HEARTBEAT_INTERVAL_MILLIS);
      final long responseTimeoutMillis = getTimeoutSettingMillis(SETTING_NAME_RESPONSE_TIMEOUT,
         DEFAULT_RESPONSE_TIMEOUT_MILLIS);
      final long reconnectTimeoutMillis = getTimeoutSettingMillis(SETTING_NAME_RECONNECT_LIMIT,
         DEFAULT_RECONNECT_LIMIT_MILLIS);


      String setting = connectSettings.getSetting("protocol");
      if (setting == null) {
         setting = System.getProperties().getProperty("FedPro.default.protocol");
         if (setting == null) {
            setting = "tcp";
         }
      }

      ClientTransportBase transport;
      switch (setting.toLowerCase()) {
         case "tls":
            // TODO: Create option to allow for TLS clients
            throw new RuntimeException("Option 'tls' not implemented.");
         case "websocket":
            transport = (ClientTransportBase) TransportFactory.createWebSocketClient();
            break;
         ...
         case "tcp":
            transport = (ClientTransportBase) TransportFactory.createTcpClient();
            break;
         default:
            throw new FedProRtiInternalError(String.format("Invalid transport type '%s' in settings", setting));
      }

      // TODO: Updating the connect settings object here is quite hacky
      connectSettings.setDefaultPort(transport.getDefaultPort());
   ...
   private long getTimeoutSettingMillis(String settingName,
                                        long defaultMillis) throws FedProRtiInternalError
   {
   ...
   @GuardedBy("_connectionStateLock")
   protected void startPersistentSession(ConnectSettings connectSettings)
      throws FedProConnectionFailed, FedProRtiInternalError, FedProNotConnected
   {
      ...
      if (connectSettings.hasSetting("ASYNC")) {
         _asyncUpdates = true;
      }

      String server = connectSettings.getServer();
      int port = connectSettings.getPort();
      final long startSessionTimeoutMillis = getTimeoutSettingMillis(SETTING_NAME_START_SESSION_TIMEOUT, DEFAULT_RESPONSE_TIMEOUT_MILLIS);

      try {
         _persistentSession.start(server, port, this::hlaCallbackRequest, startSessionTimeoutMillis, TimeUnit.MILLISECONDS);
         ...
         throw new FedProConnectionFailed("Failed to create session to server " + server + ":" + port, e);
      }
   }
   ...
   protected void disconnectBase() throws FedProRtiInternalError
   {
      synchronized (_connectionStateLock) {
         try {
   ...
   @GuardedBy("_connectionStateLock")
   private void terminatePersistentSession() throws FedProRtiInternalError
   {
      try {
