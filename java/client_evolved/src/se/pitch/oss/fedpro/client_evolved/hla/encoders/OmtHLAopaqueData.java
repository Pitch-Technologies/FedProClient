...
class OmtHLAopaqueData extends AbstractDataElement implements HLAopaqueData {
   ...
   OmtHLAopaqueData(byte[] bytes)
   {
      ...
      System.arraycopy(bytes, 0, _elements, 0, bytes.length);
   }
   ...
   public int hashCode()
   {
      return (_elements != null ? Arrays.hashCode(_elements) : 0);
   }

   static void appendByteArray(StringBuffer sb, byte[] data)
   {
      for (int i = 0; i < data.length; i++) {
         ...
         sb.append(Integer.toHexString(element).toUpperCase());
      }
   }
