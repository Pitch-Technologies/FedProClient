...
{
   private final Map<T, DataElement> _variantMap = new HashMap<T, DataElement>(10);
   ...
   private int getVariantsOctetBoundary()
   {
      if (_variantsOctetBoundary == -1) {
         ...
         for (DataElement dataElement : _variantMap.values()) {
            octetBoundary = Math.max(octetBoundary, dataElement.getOctetBoundary());
         }
   ...
   {
      if (_octetBoundary == -1) {
         _octetBoundary = Math.max(_activeDiscriminant.getOctetBoundary(), getVariantsOctetBoundary());
      }
   ...
   public boolean equals(Object o)
   {
      ...
      if (_activeDiscriminant != null ? !_activeDiscriminant.equals(omtHLAvariantRecord._activeDiscriminant) : omtHLAvariantRecord._activeDiscriminant != null) {
         return false;
      }
      if (_activeDiscriminant != null && !safeEquals(getValue(), omtHLAvariantRecord.getValue())) {
         return false;
      }
   ...
   {
      StringBuffer sb = new StringBuffer("HLAvariantRecord<");
