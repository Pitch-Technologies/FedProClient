...
public class CircularBuffer<E> implements GenericBuffer<E> {
   ...
   @GuardedBy("_lock") private final E[] _data;
   @GuardedBy("_lock") private int _writeIndex;
   @GuardedBy("_lock") private int _count;
   ...
   @Override
   public E waitAndPoll() throws InterruptedException
   {
      synchronized (_lock) {
         while (isEmpty()) {
            _lock.wait();
   ...
   @Override
   public E waitAndPeek() throws InterruptedException
   {
