...
public abstract class MessageWriter {
   ...
   @GuardedBy("_sessionLock") private final GenericBuffer<QueueableMessage> _messageQueue;

   public MessageWriter(long sessionId,
   ...
   protected void addMessage(int lastReceivedSequenceNumber,
                             MessageType messageType)
   {
      addMessage(0, lastReceivedSequenceNumber, messageType, null);
   }

   protected void addMessage(long payloadSize,
   ...
   protected CompletableFuture<byte[]> addRequest(int lastReceivedSequenceNumber,
                                                  MessageType messageType,
   ...
   protected CompletableFuture<byte[]> addRequest(long payloadSize,
                                                  int lastReceivedSequenceNumber,
                                                  MessageType messageType,
                                                  EncodableMessage message,
                                                  Map<Integer, CompletableFuture<byte[]>> futuresMap)
   ...
   private CompletableFuture<byte[]> addRequest(long payloadSize,
                                                int lastReceivedSequenceNumber,
                                                MessageType messageType,
                                                EncodableMessage message,
                                                Map<Integer, CompletableFuture<byte[]>> futuresMap,
