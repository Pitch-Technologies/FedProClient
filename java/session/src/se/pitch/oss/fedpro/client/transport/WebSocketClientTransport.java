...
public class WebSocketClientTransport extends ClientTransportBase {
   public WebSocketClientTransport()
   {
      super(Ports.DEFAULT_PORT_WS);
   }

   public WebSocketClientTransport(int port)
   {
      super(port);
   }
   ...
   protected String addHostPrefix(String host) throws UnknownHostException
   {
      ...
      if (hasProtocol) {
         ...
         if (!protocol.equals("ws")) {
            throw new UnknownHostException(String.format("Invalid protocol '%s' used together with WebSocket, expected 'ws'.",
               protocol));
         }
      } else {
         host = "ws://" + host;
