...
public class SessionImpl implements Session {
   ...
   private static final int MESSAGE_QUEUE_SIZE =
         Integer.getInteger("se.pitch.oss.prti1516e.fedpro.client.message.queue", 2000);
   ...
   private static final boolean ALLOW_GAPS_IN_SEQUENCE_NUMBERS =
         Boolean.getBoolean("se.pitch.oss.prti1516e.fedpro.common.allowGapsInSequenceNumbers");
   ...
   throws SessionIllegalState
   {
      synchronized (_sessionLock) {
         if (_state.equals(State.NEW)) {
            ...
            throw new SessionIllegalState("Session timeout can only be set before session start. Session state is " + _state);
         }
      }
   }
   ...
   public State compareAndSetState(Map<State, State> transitions, String reason) {
      synchronized (_sessionLock) {
         ...
         State newState)
   {
      ...
      } else if (oldState == State.TERMINATING) {
   ...
   throws SessionLost, SessionIllegalState
   {
      ...
      doSessionOperation(() -> {
         if (getState() == State.RESUMING) {
            try {
               waitForStates(State.RUNNING, State.DROPPED, State.TERMINATED);
            } catch (InterruptedException ignore) {
            }
         }
         compareAndSetState(Map.ofEntries(
   ...
   private void runMessageReaderLoop()
   {
      try {
         ...
         compareAndSetState(Map.ofEntries(
               entry(State.RUNNING, State.DROPPED),
               entry(State.TERMINATING, State.TERMINATED)
         ), e.toString());
         LOGGER.info(() -> String.format("%s: Session dropped: %s", logPrefix(), e));
      } catch (IOException e) {
         State oldState = compareAndSetState(Map.ofEntries(
               entry(State.RUNNING, State.DROPPED),
               entry(State.TERMINATING, State.TERMINATED)
         ), e.toString());
