...
import se.pitch.oss.fedpro.client.transport.ClientTransportBase;
...
import static se.pitch.oss.fedpro.common.session.MessageType.*;

public class SessionImpl implements Session {
   private static final Logger LOGGER = Logger.getLogger(SessionImpl.class.getName());

   private static final int MESSAGE_QUEUE_SIZE = Integer.getInteger("se.pitch.oss.prti1516e.fedpro.client.message.queue", 2000);
   private static final boolean RATE_LIMIT_ENABLED = Boolean.getBoolean("se.pitch.oss.prti1516.fedpro.client.rate.limit");
   ...
   private final ClientTransportBase _transport;
   ...
   public SessionImpl(ClientTransport transport)
   {
      _transport = (ClientTransportBase) transport;
   }
   ...
   public void setSessionTimeout(long sessionTimeout,
                                 TimeUnit sessionTimeoutUnit) throws SessionIllegalState
   {
      synchronized (_sessionLock) {
   ...
   private State waitForState(State state,
                              long timeout,
   ...
   private State waitForStates(long timeout,
                               TimeUnit timeUnit,
                               State... anticipatedStates) throws InterruptedException
   {
      final long endTime = System.currentTimeMillis() + TimeUnit.MILLISECONDS.convert(timeout, timeUnit);
      synchronized (_sessionLock) {
         long timeLeft = endTime - System.currentTimeMillis();
   ...
   private State compareAndSetState(State expectedState,
                                    State newState)
   {
   ...
   @GuardedBy("_sessionLock")
   private void transitionSessionState(State oldState,
                                       State newState)
   {
   ...
   private void fireOnStateTransition(State oldState,
                                      State newState,
                                      String reason)
   {
   ...
   public void start(String host,
                     int port,
                     HlaCallbackRequestListener hlaCallbackRequestListener) throws SessionLost, SessionIllegalState
   {
      start(host, port, hlaCallbackRequestListener, DEFAULT_RESPONSE_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS);
   }
   ...
   public void start(String host,
                     ...
                     TimeUnit connectionTimeoutUnit) throws SessionLost, SessionIllegalState
   {
      ...
      GenericBuffer<QueueableMessage> messageQueue = new RoundRobinBuffer<>(MESSAGE_QUEUE_SIZE,
         limiter,
      ...
      try {
         _socket = _transport.connect(_host, _port);

         _socketWriter = new SocketWriter(MessageHeader.NO_SESSION_ID,
         ...
         final TimeoutTimer connectionTimeoutTimer =
            TimeoutTimer.createLazyTimeoutTimer("Client Connection Timeout Timer", connectionTimeoutMillis);
      ...
      _sessionTimeoutTimer = TimeoutTimer.createLazyTimeoutTimer("Client Session Timeout Timer", _sessionTimeout);
   ...
   {
      LOGGER.severe(() -> String.format("%s: No response from server in %d milliseconds, session timing out.",
   ...
   private SocketWriter.Listener getSocketWriterListener()
   {
      return new SocketWriter.Listener() {
         @Override
         public void messagesLost(String reason)
         {
            LOGGER.severe(() -> String.format("%s: Lost messages before sending, session will terminate. Reason: %s",
               logPrefix(),
               reason));
         }
   ...
   @Override
   public boolean resume() throws SessionLost, IOException, SessionIllegalState
   {
      State beforeState = compareAndSetState(State.DROPPED, State.RESUMING);
      switch (beforeState) {
         case DROPPED:
            // Correct state, continue resuming.
            break;
         case NEW:
         case STARTING:
            throw new SessionIllegalState("Tried to resume uninitialized session.");
         case TERMINATED:
            throw new SessionIllegalState("Tried to resume terminated session.");
         default:
            throw new SessionIllegalState("Tried to resume session that was in state " + beforeState);
      }
      ...
      LOGGER.fine(() -> String.format("%s: Oldest available Federate message %d. Last received RTI message is %d.",
         logPrefix(),
         oldestAvailableFederateMessage,
         lastReceivedRtiMessage));

      SessionSocket newSocket = null;
      try {

         newSocket = _transport.connect(_host, _port);

         SocketWriter directSocketWriter = SocketWriter.directSocketWriter(_sessionId, newSocket, true);
         EncodedMessage resumeSessionMessage = ClientMessageWriter.createResumeSessionMessage(_sessionId,
            lastReceivedRtiMessage,
            oldestAvailableFederateMessage);
         ...
         if (resumeStatusMessage.sessionStatus != ResumeStatusMessage.STATUS_OK_TO_RESUME) {
            throw new SessionLost(
               "Server refused to resume session. Reason: " + resumeStatusToString(resumeStatusMessage.sessionStatus));
         }

         SequenceNumber messageToResumeFrom = new SequenceNumber(resumeStatusMessage.lastReceivedFederateSequenceNumber);
         messageToResumeFrom.increment();

         LOGGER.fine(() -> String.format("%s: Resuming Federate messages from sequence number %d.",
            logPrefix(),
            messageToResumeFrom.get()));

         _socketWriter.rewindToSequenceNumber(messageToResumeFrom);
   ...
   @Override
   public CompletableFuture<byte[]> sendHeartbeat() throws SessionIllegalState
   {
      return doSessionOperation(() -> _messageWriter.writeHeartbeatMessage(_lastReceivedSequenceNumber.get(),
         _requestFutures));
   }

   @Override
   public CompletableFuture<byte[]> sendHlaCallRequest(byte[] encodedHlaCall) throws SessionIllegalState
   {
      return doSessionOperation(() -> _messageWriter.writeHlaCallRequest(encodedHlaCall,
         _lastReceivedSequenceNumber.get(),
         _requestFutures));
   }

   @Override
   public void sendHlaCallbackResponse(int responseToSequenceNumber,
                                       byte[] hlaCallbackResponse) throws SessionIllegalState
   {
      doSessionOperation(() -> _messageWriter.writeHlaCallbackResponse(responseToSequenceNumber,
         hlaCallbackResponse,
         _lastReceivedSequenceNumber.get()));
   }

   @Override
   public void terminate() throws SessionLost, SessionIllegalState
   {
      terminate(((int) _sessionTimeout), TimeUnit.MILLISECONDS);
   }

   @Override
   public void terminate(int responseTimeout, TimeUnit responseTimeoutUnit) throws SessionLost, SessionIllegalState
   {
      ...
      try {
         ...
         throw new SessionLost(logPrefix() + ": Interrupted while waiting for response to termination request: " + e,
            e);
   ...
   private CompletableFuture<byte[]> doSessionOperation(AsyncSessionOperation operation) throws SessionIllegalState
   {
      synchronized (_sessionLock) {
   ...
   private void doSessionOperation(Runnable operation) throws SessionIllegalState
   {
      doSessionOperation(() -> {
         operation.run();
   ...
   private void close(SessionSocket socket)
   {
      if (socket != null) {
         try {
            socket.close();
   ...
   {
      _socketWriterThread = new Thread(_socketWriter::socketWriterLoop,
   ...
   private void runMessageReaderLoop()
   {
      try {
            ...
            switch (messageHeader.messageType) {
               ...
               case CTRL_SESSION_TERMINATED: {
                  ...
                  if (future != null) {
                     ...
                     LOGGER.warning(() -> String.format("%s: Received unexpected CTRL_SESSION_TERMINATED.",
                        logPrefix()));
                  }
               ...
               case HLA_CALL_RESPONSE: {
                  HlaCallResponseMessage message = HlaCallResponseMessage.decode(inputStream,
               ...
               case HLA_CALLBACK_REQUEST: {
                  HlaCallbackRequestMessage message = HlaCallbackRequestMessage.decode(inputStream,
                     messageHeader.getPayloadSize());
                  ...
                  _hlaCallbackRequestListener.onHlaCallbackRequest(messageHeader.sequenceNumber,
         ...
         compareAndSetState(State.RUNNING, State.DROPPED, e.toString());
         LOGGER.severe(() -> String.format("%s: Got unexpected exception in message reader thread: %s",
            logPrefix(),
            e));
      }
   }
   ...
   private void trackMessageReceived(int sequenceNumber)
   {
      ...
      if (SequenceNumber.isValidAsSequenceNumber(sequenceNumber)) {
         ...
         if (!ALLOW_GAPS_IN_SEQUENCE_NUMBERS && sequenceNumber != previousReceived.getNext()) {
            LOGGER.warning(() -> String.format("%s: Got unexpected sequence number %d. Last received was %d.",
               logPrefix(),
               sequenceNumber,
   ...
   private long readNewSessionStatus(InputStream inputStream) throws IOException, BadMessage, SessionLost
   {
      MessageHeader messageHeader = MessageHeader.decode(inputStream);
      logReceivedMessage(messageHeader.messageType, messageHeader.sequenceNumber);
   ...
   private ResumeStatusMessage readResumeStatus(InputStream inputStream) throws IOException, BadMessage
   {
      MessageHeader messageHeader = MessageHeader.decode(inputStream);
      _sessionTimeoutTimer.extend();
      logReceivedMessage(messageHeader.messageType, messageHeader.sequenceNumber);
   ...
   private void throwAsSessionException(Exception e) throws SessionLost
   {
   ...
   private void logReceivedMessage(MessageType messageType,
                                   int sequenceNumber)
   {
      LOGGER.finer(() -> String.format("%s: Received seq.nr. %s, type %s",
         logPrefix(),
