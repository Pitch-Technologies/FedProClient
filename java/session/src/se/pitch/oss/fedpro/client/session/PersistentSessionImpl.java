...
public class PersistentSessionImpl implements PersistentSession {
    ...
    */
   public PersistentSessionImpl(ClientTransport transportProtocol,
                                    ...
                                    TimeUnit heartbeatIntervalUnit)
   {
      if (_sendHeartbeatTimer == null) {
         ...
         LOGGER.warning(() -> String.format("%s: Heartbeat interval can only be set before session start. ", logPrefix()));
      }
   }
   ...
   {
      _session.start(host, port, hlaCallbackRequestListener);
      startHeartBeatTimer();
   }
   ...
   {
      _session.start(host, port, hlaCallbackRequestListener, connectionTimeout, connectionTimeoutUnit);
      startHeartBeatTimer();
   }

   private void startHeartBeatTimer()
   {
      _sendHeartbeatTimer = TimeoutTimer.createEagerTimeoutTimer("Client Session Heartbeat Timer", _heartbeatIntervalUnit.toMillis(_heartbeatInterval));
      _session.setMessageSentListener(_sendHeartbeatTimer::extend);
      _sendHeartbeatTimer.start(this::heartbeat);
   }

   @Override
   public CompletableFuture<byte[]> sendHlaCallRequest(byte[] encodedCall) throws SessionIllegalState
   {
      return _session.sendHlaCallRequest(encodedCall);
   }
   ...
   private void heartbeat()
   {
      ...
      try {
         ...
         LOGGER.log(Level.SEVERE, e, () -> String.format("%s: Unexpected exception in session heartbeat timer.", logPrefix()));
      }
   }
   ...
   private void sleepMillis(long timeMillis) throws InterruptedException
   {
      if (timeMillis > 0) {
