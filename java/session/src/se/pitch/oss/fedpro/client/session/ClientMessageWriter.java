...
public class ClientMessageWriter extends MessageWriter {
   public ClientMessageWriter(long sessionId,
   ...
   public CompletableFuture<byte[]> writeHeartbeatMessage(int lastReceivedSequenceNumber,
   ...
   public static EncodedMessage createResumeSessionMessage(long sessionId,
                                                           int lastReceivedSequenceNumber,
                                                           int oldestAvailableSequenceNumber)
   {
      return EncodedMessage.create(
         MessageHeader.with(4 + 4,
            SequenceNumber.NO_SEQUENCE_NUMBER,
            sessionId,
   ...
   public CompletableFuture<byte[]> writeHlaCallRequest(byte[] hlaServiceCallWithParams,
                                                        int lastReceivedSequenceNumber,
                                                        Map<Integer, CompletableFuture<byte[]>> futuresMap)
   {
      return addRequest(hlaServiceCallWithParams.length,
         lastReceivedSequenceNumber,
   ...
   public void writeHlaCallbackResponse(int responseToSequenceNumber,
                                        byte[] encodedResponse,
                                        int lastReceivedSequenceNumber)
   {
      addMessage(4 + encodedResponse.length,
         lastReceivedSequenceNumber,
