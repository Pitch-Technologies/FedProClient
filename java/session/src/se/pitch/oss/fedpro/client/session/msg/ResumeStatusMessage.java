...
public class ResumeStatusMessage implements EncodableMessage {
   ...
   public ResumeStatusMessage(int sessionStatus,
                              int lastReceivedFederateSequenceNumber)
   {
   ...
   public static ResumeStatusMessage decode(InputStream inputStream) throws BadMessage, IOException
   {
      ...
      if (!(sessionStatus == STATUS_OK_TO_RESUME || sessionStatus == STATUS_FAILURE_INVALID_SESSION ||
         sessionStatus == STATUS_FAILURE_NOT_ENOUGH_BUFFERED_MESSAGES || sessionStatus == STATUS_FAILURE_OTHER_ERROR)) {
