...
// TODO: Decide whether to include this sample with installation.
public class AsyncChatAdvanced extends NullFederateAmbassador {
   ...
   private void run() throws Exception {
      ...
      _reservationFuture
         .thenCompose(ignoredVoid -> _asyncRtiAmbassador.registerObjectInstanceWithName(setupObjectsFuture.join(), _username))
   ...
   private void connectAndJoinBlockingly() throws Exception {
      String rtiHost;
      if (_args.size() > 0) {
         rtiHost = _args.get(0);
      } else {
   ...
   private CompletableFuture<InteractionClassHandle> prepInteractionClass(InteractionClassHandle communicationClassHandle) {
      ...
      return CompletableFuture.allOf(messageHandleFuture, senderHandleFuture, subscribeInteractionFuture, publishInteractionFuture)
   ...
   private CompletableFuture<ObjectClassHandle> prepObjectClassWithAttributes(ObjectClassHandle participantClassHandle,
      ...
      try {
         ...
         CompletableFuture<Void> subscribeObjectFuture = _asyncRtiAmbassador.subscribeObjectClassAttributes(participantClassHandle, attributeSet);
         CompletableFuture<Void> publishObjectFuture = _asyncRtiAmbassador.publishObjectClassAttributes(participantClassHandle, attributeSet);
